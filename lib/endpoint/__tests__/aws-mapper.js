...
describe('mappers.aws-mapper.js', () => {
  ...
  test('convertContextToResponse should map event to an object req can consume', () => {
    ...
    const mappedObject = awsMapper.apply({}, [event, context]);
    expect(mappedObject.req).toBeInstanceOf(Req);
    expect(mappedObject.res).toBeInstanceOf(Res);
  });

  test('calling awsMapper with apply can receive options', () => {
    const context = { succeed: function succeed() {}, fail: function fail() {} };
    const event = eventObject;
    const message = awsMapper.convertContextToResponse(context);
    const configOptions = { options: {} };

    awsMapper.apply(configOptions, [event, context]);

    expect(Res).toBeCalledWith(message, configOptions.options);
  });
